...
class RLHFDataset(Dataset):
    ...
    def _resolve_media_paths(self, paths: list[str]) -> list[str]:
        """
        Resolve media paths with `image_dir`.

        Supports:
          - absolute paths that already exist (kept as-is)
          - OmniMedVQA root-relative paths like "/Images/..." (joined as image_dir + "Images/...")
          - relative paths like "Images/..." (joined as image_dir + "Images/...")
        """
        ...
        for p in paths:
            if os.path.isabs(p):
                if os.path.exists(p):
                    resolved.append(p)
                elif p.startswith(("/Images/", "/Videos/", "/images/", "/videos/")):
                    resolved.append(os.path.join(self.image_dir, p.lstrip("/")))
                else:
                    # Keep original behavior for other absolute paths.
                    resolved.append(p)
            else:
                resolved.append(os.path.join(self.image_dir, p))
        return resolved
    ...
    def _filter_overlong_prompts(self, example: dict[str, Any]) -> bool:
        ...
        if self.image_key in example:
            ...
            if self.image_dir is not None and len(images) != 0 and isinstance(images[0], str):  # image paths
                images = self._resolve_media_paths(images)
            ...
            if self.image_dir is not None and len(videos) != 0 and isinstance(videos[0], str):  # video paths
                videos = self._resolve_media_paths(videos)

...
Outputs clean JSONL with only sample information.
Prompts are generated dynamically via jinja templates.
"""
...
def load_data(data_path: str, image_root: str = None) -> tuple[dict[str, list], list]:
...
def generate_single_sample(sample: dict) -> dict:
    """Generate single-image sample with task_type marker."""
    # Unwrap the single image path from the array
    image_path = sample["images"][0] if sample.get("images") else None

    # Generate prompt with <image> placeholder
...
def generate_comparative_sample(
    ...
    """Generate multi-image comparative sample."""
...
def generate_dataset(
    ...
    """Generate train and val datasets with structured data only."""
    random.seed(seed)

    # Load data
    samples_by_split_class, _ = load_data(data_path, image_root)
...
def main():
    ...
    parser.add_argument("--image_root", type=str, default=None, help="Root path to prepend to all image paths")
    ...
    print("=" * 60)
    print("Structured Data Generation for Comparative Training")
    print("=" * 60)
    ...
    print("\nDone!")
    print("\nRemember to use the jinja template for prompts!")
